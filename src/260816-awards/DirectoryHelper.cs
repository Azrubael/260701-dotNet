...
public static partial class DirectoryHelper
{
    ...
    public static string?[] FindFiles(string dirPath)
    {
        ...
        try
        {
            // os.listdir(dir_path) returns names (entries) inside the directory
            var entries = Directory.GetFileSystemEntries(dirPath);

            // Convert full paths -> just names (like os.listdir)
            var names = Array.ConvertAll(entries, Path.GetFileName);
    ...
    public static Dictionary<string, string> CheckMatching(string[] xlsxFiles, string directoryPath)
    {
        ...
        foreach (var file in matchingFiles)
        {
            ...
            if (dateStr.Length == 6 && dateStr.All(char.IsDigit))
            {
                try
                {
                    // Python: datetime.strptime(date_str, '%y%m%d') then format '%Y-%m-%d'
                    // In C#: DateTime.ParseExact with format "yyMMdd"
