...
public partial class MainWindow : Window
{
  ...
  private void OnNewGameClick(object? sender, RoutedEventArgs e)
  {
    ...
    NextPieceBorder.IsVisible = true;
  ...
  private void DrawNextTetromino()
  {
    ...
    var cells = Game.NextPiece.GetCells();
    ...
    int pieceWidth = (maxX - minX + 1) * CellSize;
    int pieceHeight = (maxY - minY + 1) * CellSize;

    int offsetX = ((int)NextPieceCanvas.Width - pieceWidth) / 2;
    int offsetY = ((int)NextPieceCanvas.Height - pieceHeight) / 2;

    var color = Tetromino.GetColor(Game.NextPiece.Type);
    ...
    foreach (var (x, y) in cells)
    {
      ...
      Canvas.SetLeft(cell, offsetX + (x - minX) * CellSize);
      Canvas.SetTop(cell, offsetY + (y - minY) * CellSize);

      NextPieceCanvas.Children.Add(cell);
    }
  }
